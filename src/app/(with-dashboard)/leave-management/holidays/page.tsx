...
export default function HolidaysPage() {
  ...
  // TODO: Replace with actual holiday data from GraphQL
  ...
  // Calculate stats
  const totalHolidays = dummyHolidays.length;
  const paidHolidays = dummyHolidays.filter((h) => h.isPaid).length;
  const recurringHolidays = dummyHolidays.filter((h) => h.isRecurring).length;
  const upcomingHolidays = dummyHolidays.filter((h) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <CustomTable
        ...
        dataSource={dummyHolidays.map((row) => ({
          ...row,
          customName: row.name,
          customHolidayPeriod:
